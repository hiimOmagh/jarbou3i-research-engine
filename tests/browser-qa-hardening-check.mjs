...
const docs = fs.existsSync('docs/v1.0.5-browser-qa-visual-regression-hardening.md') ? fs.readFileSync('docs/v1.0.5-browser-qa-visual-regression-hardening.md', 'utf8') : '';
...
for (const script of ['test:browser:layout','test:browser:visual','test:browser:visual:strict','test:browser:qa','test:v105:no-browser','test:v105']) {
...
for (const token of ['VISUAL_BASELINE_STRICT','captureOrCompare','toHaveScreenshot','v105-desktop-${tab}','v105-mobile-analysis']) {
...
assert.ok(docs.includes('Browser QA + Visual Regression Hardening'), 'v1.0.5 docs missing');
assert.equal(fixture.workflow_version, '1.0.5');
assert.equal(fixture.browser_qa_hardening?.hardening_version, '1.0.5');
