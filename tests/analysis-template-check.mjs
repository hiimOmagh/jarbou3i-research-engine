...
assert.equal(templates.VERSION, '0.27.0-beta');
...
for (const id of ['strategic_analysis_engine','geopolitical_event_analysis','policy_impact_analysis','market_technology_trend_analysis','actor_incentive_map','contradiction_audit','scenario_forecast']) {
  ...
  assert.equal(profile.template_version, '0.27.0-beta');
...
assert.equal(fit.fit_version, '0.27.0-beta');
...
assert.equal(fixture.analysis_brief.template_fit_report.fit_version, '0.27.0-beta');
