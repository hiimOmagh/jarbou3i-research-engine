...
assert.ok(renderHelpers.includes("alphaBadge:'v1.0.6 · Documentation + Release Packaging Cleanup'"), 'English stable badge must be fixed');
assert.ok(renderHelpers.includes('تنظيف التوثيق وحزمة الإصدار'), 'Arabic release badge must be fixed');
assert.ok(renderHelpers.includes('Nettoyage documentation et paquet de publication'), 'French release badge must be fixed');
