...
assert.ok(ciNoBrowser.includes('tests/v104-no-browser-suite.mjs'), 'CI no-browser syntax gate must target v104 suite');
...
assert.ok(renderHelpers.includes("alphaBadge:'v1.0.4 · Public Beta / Stable Research Engine'"), 'English stable badge must be fixed');
...
assert.ok(pkg.scripts['test:v104:no-browser'].includes('v104-no-browser-suite'), 'package must expose v104 no-browser suite');
