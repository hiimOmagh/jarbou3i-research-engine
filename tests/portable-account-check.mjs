...
const disconnected = mod.disconnected('0.26.0-beta');
...
const connected = mod.connect({version:'0.26.0-beta', accountHint:'fixture'});
...
const status = mod.status(connected, {version:'0.26.0-beta'});
if(!status.connected || !status.token_present) fail('status must mark connected token presence');
const refreshed = mod.refresh(connected, {version:'0.26.0-beta'});
if(refreshed.token_hash === connected.token_hash) fail('refresh should rotate mock token hash');
const exported = mod.exportableStatus(refreshed, {version:'0.26.0-beta'});
