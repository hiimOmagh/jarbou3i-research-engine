...
const disconnected = mod.disconnected('0.29.0-rc.1');
...
const connected = mod.connect({version:'0.29.0-rc.1', accountHint:'fixture'});
...
const status = mod.status(connected, {version:'0.29.0-rc.1'});
if(!status.connected || !status.token_present) fail('status must mark connected token presence');
const refreshed = mod.refresh(connected, {version:'0.29.0-rc.1'});
if(refreshed.token_hash === connected.token_hash) fail('refresh should rotate mock token hash');
const exported = mod.exportableStatus(refreshed, {version:'0.29.0-rc.1'});
