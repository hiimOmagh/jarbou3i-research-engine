...
for (const file of tests) {
  try {
    ...
    console.error(`v0.24 no-browser suite failed while running ${file}: ${error.message}`);
    process.exit(error.status || 1);
  }
}
console.log('v0.24 no-browser suite passed. Run npm run test:browser:provider after Playwright browser install.');
process.exit(0);
