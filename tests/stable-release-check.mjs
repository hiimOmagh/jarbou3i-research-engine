...
assert.equal(release.RELEASE_VERSION, '1.0.0');
...
assert.ok(index.includes('v1.0.0 · Public Beta / Stable Research Engine'), 'stable badge missing');
...
assert.ok(migrationSource.includes('1.0.0'), 'v1.0.0 target must be supported');

const policy = release.releaseCandidatePolicy({ version: '1.0.0' });
...
}, { version: '1.0.0', externalCiPassed: true, externalCiNotes: 'external CI succeeded' });
assert.equal(report.release_candidate_version, '1.0.0');
assert.equal(report.stable_release_version, '1.0.0');
...
assert.equal(fixture.workflow_version, '1.0.0');
assert.equal(fixture.release_candidate.release_candidate_version, '1.0.0');
assert.equal(fixture.release_candidate.stable_release_version, '1.0.0');
...
assert.equal(pkg.scripts['test:stable'], 'node tests/stable-release-check.mjs');
assert.ok(pkg.scripts['test:v100:no-browser'].includes('v100-no-browser-suite'));
