...
assert.equal(migrations.TARGET_VERSION, '0.25.0-beta');
...
for (const file of fixtures) {
  ...
  const result = migrations.migrateResearchPacket(input, { targetVersion: '0.25.0-beta' });
  assert.equal(result.ok, true, `${file} should migrate`);
  assert.equal(result.packet.workflow_version, '0.25.0-beta', `${file} workflow version`);
  assert.equal(result.packet.research_plan.plan_version, '0.25.0-beta', `${file} plan version`);
  ...
  assert.equal(result.packet.analysis_template.template_version, '0.25.0-beta', `${file} analysis template version`);
  assert.equal(result.packet.analysis_template.template_id, 'strategic_analysis_engine', `${file} default analysis template`);
  assert.equal(result.packet.quality_gate.quality_gate_version, '0.25.0-beta', `${file} quality gate version`);
  assert.equal(result.packet.export_pack.export_pack_version, '0.25.0-beta', `${file} export pack version`);
  assert.equal(result.packet.export_pack.format, 'export_pack_v2', `${file} export pack format`);
  assert.equal(result.packet.backend_hardening.hardening_version, '0.25.0-beta', `${file} backend hardening version`);
  ...
  assert.equal(result.packet.packet_migration_report.target_version, '0.25.0-beta', `${file} report target`);
...
const redacted = migrations.migrateResearchPacket(unsafeLegacy, { targetVersion: '0.25.0-beta' });
