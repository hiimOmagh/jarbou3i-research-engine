...
assert.equal(migrations.TARGET_VERSION, '0.17.0-beta');
...
const secretLike = /(sk-[A-Za-z0-9_-]{12,}|Bearers+[A-Za-z0-9._~+/=-]{12,}|raw-token-should-redact|ya29.[A-Za-z0-9._-]{20,}|ghp_[A-Za-z0-9_]{16,})/i;
...
for (const file of fixtures) {
  ...
  const result = migrations.migrateResearchPacket(input, { targetVersion: '0.17.0-beta' });
  assert.equal(result.ok, true, `${file} should migrate`);
  assert.equal(result.packet.workflow_version, '0.17.0-beta', `${file} workflow version`);
  assert.equal(result.packet.research_plan.plan_version, '0.17.0-beta', `${file} plan version`);
  ...
  assert.equal(result.packet.packet_migration_report.target_version, '0.17.0-beta', `${file} report target`);
...
const redacted = migrations.migrateResearchPacket(unsafeLegacy, { targetVersion: '0.17.0-beta' });
