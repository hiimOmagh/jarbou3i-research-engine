...
assert.equal(migrations.TARGET_VERSION, '1.0.2');
...
for (const file of fixtures) {
  ...
  const result = migrations.migrateResearchPacket(input, { targetVersion: '1.0.2' });
  assert.equal(result.ok, true, `${file} should migrate`);
  assert.equal(result.packet.workflow_version, '1.0.2', `${file} workflow version`);
  assert.equal(result.packet.research_plan.plan_version, '1.0.2', `${file} plan version`);
  ...
  assert.equal(result.packet.analysis_template.template_version, '1.0.2', `${file} analysis template version`);
  assert.equal(result.packet.analysis_template.template_id, 'strategic_analysis_engine', `${file} default analysis template`);
  assert.equal(result.packet.quality_gate.quality_gate_version, '1.0.2', `${file} quality gate version`);
  assert.equal(result.packet.release_candidate.release_candidate_version, '1.0.2', `${file} release candidate version`);
  ...
  assert.equal(result.packet.export_pack.export_pack_version, '1.0.2', `${file} export pack version`);
  assert.equal(result.packet.export_pack.format, 'export_pack_v2', `${file} export pack format`);
  assert.equal(result.packet.backend_hardening.hardening_version, '1.0.2', `${file} backend hardening version`);
  ...
  assert.equal(result.packet.packet_migration_report.target_version, '1.0.2', `${file} report target`);
...
const redacted = migrations.migrateResearchPacket(unsafeLegacy, { targetVersion: '1.0.2' });
