...
for (const id of ['researchLabPanel','generatePlanBtn','copyPlanPromptBtn','evClaim','addEvidenceBtn','cancelEvidenceEditBtn','exportWorkflowBtn','importWorkflowInput','addCausalLinkBtn','inferCausalLinksBtn','causalLinksOutput','compileBriefBtn','copySynthesisPromptBtn','analysisBriefOutput','validationDiagnosticsOutput','providerName','providerTask','runProviderTaskBtn','copyProviderPayloadBtn','exportRunLedgerBtn','clearRunLedgerBtn','providerRunOutput','generateMockAnalysisBtn','runMockRepairBtn','runCritiqueBtn','researchQualityOutput']) {
  if (!index.includes(`id="${id}"`)) fail(`missing research UI id: ${id}`);
}
for (const token of ['buildResearchPlan','compileAnalysisBrief','buildSynthesisPrompt','buildSourceClusters','diagnosticReport','responseContract','buildProviderPayload','mockProviderResponse','runProviderTask','buildMockAnalysis','buildCritique','evidence_matrix','causal_links','analysis_brief','research_plan','Quality Gate v2']) {
...
if (schema.properties?.workflow_version?.const !== '0.4.0-alpha') fail('workflow_version const mismatch');
...
if (fixture.workflow_version !== '0.4.0-alpha') fail('fixture workflow version mismatch');
if (!plan || plan.plan_version !== '0.4.0-alpha') fail('fixture missing alpha research plan');
...
if (!fixture.analysis_brief || fixture.analysis_brief.brief_version !== '0.4.0-alpha') fail('fixture needs compiled analysis brief');
...
if (!fixture.diagnostics || fixture.diagnostics.diagnostics_version !== '0.4.0-alpha') fail('fixture needs diagnostics');
...
if (!fixture.ai_runs.every((run) => run.run_version === '0.4.0-alpha' && run.provider && run.task && run.response_contract)) fail('fixture provider runs need contracts');
