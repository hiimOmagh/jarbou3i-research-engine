...
if (schema.properties?.workflow_version?.const !== '0.13.0-beta') fail('workflow_version const mismatch');
for (const key of ['workflow_version','research_plan','evidence_matrix','analysis_brief','diagnostics','ai_runs','source_policy','source_diagnostics','source_fixture_report','source_requests','source_runs']) {
...
if (fixture.workflow_version !== '0.13.0-beta') fail('fixture workflow version mismatch');
if (!plan || plan.plan_version !== '0.13.0-beta') fail('fixture missing alpha research plan');
...
if (!fixture.analysis_brief || fixture.analysis_brief.brief_version !== '0.13.0-beta') fail('fixture needs compiled analysis brief');
...
if (!fixture.diagnostics || fixture.diagnostics.diagnostics_version !== '0.13.0-beta') fail('fixture needs diagnostics');
...
if (!fixture.ai_runs.every((run) => run.run_version === '0.13.0-beta' && run.provider && run.task && run.response_contract && run.response_validation && run.repair_trace && run.provider_safety)) fail('fixture provider runs need contracts and safety metadata');
