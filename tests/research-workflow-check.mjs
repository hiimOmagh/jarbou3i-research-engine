...
if (schema.properties?.workflow_version?.const !== '0.22.0-beta') fail('workflow_version const mismatch');
for (const key of ['workflow_version','analysis_template','project_workspace','research_plan','evidence_matrix','analysis_brief','diagnostics','provider_identity','provider_billing_policy','privacy_export','ai_runs','source_policy','source_diagnostics','source_fixture_report','source_requests','source_runs']) {
  if (!schema.required?.includes(key)) fail(`research schema must require ${key}`);
}
if (!fixture.analysis_template || fixture.analysis_template.template_version !== '0.22.0-beta' || fixture.analysis_template.template_id !== 'strategic_analysis_engine') fail('fixture needs analysis template metadata');
const plan = fixture.research_plan;
const evidence = fixture.evidence_matrix;
const causalLinks = fixture.causal_links;
if (fixture.workflow_version !== '0.22.0-beta') fail('fixture workflow version mismatch');
if (!plan || plan.plan_version !== '0.22.0-beta' || plan.analysis_template_id !== fixture.analysis_template.template_id) fail('fixture missing templated research plan');
...
if (!fixture.analysis_brief || fixture.analysis_brief.brief_version !== '0.22.0-beta' || fixture.analysis_brief.analysis_template?.template_id !== fixture.analysis_template.template_id || !fixture.analysis_brief.template_fit_report) fail('fixture needs templated compiled analysis brief');
...
if (!fixture.diagnostics || fixture.diagnostics.diagnostics_version !== '0.22.0-beta') fail('fixture needs diagnostics');
...
if (!fixture.project_workspace || fixture.project_workspace.workspace_version !== '0.22.0-beta' || fixture.project_workspace.storage_mode !== 'local_only' || !fixture.project_workspace.quota) fail('fixture needs local-only project workspace metadata');

if (!fixture.privacy_export || fixture.privacy_export.guard_version !== '0.22.0-beta' || fixture.privacy_export.key_exported !== false || fixture.privacy_export.raw_token_exported !== false) fail('fixture needs privacy export guard metadata');
...
if (!fixture.ai_runs.every((run) => run.run_version === '0.22.0-beta' && run.provider && run.task && run.response_contract && run.response_validation && run.repair_trace && run.provider_safety && run.provider_identity && run.provider_billing_policy)) fail('fixture provider runs need contracts, identity, billing, and safety metadata');
