...
for (const id of ['researchLabPanel','generatePlanBtn','copyPlanPromptBtn','evClaim','addEvidenceBtn','cancelEvidenceEditBtn','exportWorkflowBtn','importWorkflowInput','addCausalLinkBtn','inferCausalLinksBtn','causalLinksOutput','compileBriefBtn','copySynthesisPromptBtn','analysisBriefOutput','validationDiagnosticsOutput','generateMockAnalysisBtn','runMockRepairBtn','runCritiqueBtn','researchQualityOutput']) {
  if (!index.includes(`id="${id}"`)) fail(`missing research UI id: ${id}`);
}
for (const token of ['buildResearchPlan','compileAnalysisBrief','buildSynthesisPrompt','buildSourceClusters','diagnosticReport','buildMockAnalysis','buildCritique','evidence_matrix','causal_links','analysis_brief','research_plan','Quality Gate v2']) {
...
if (schema.properties?.workflow_version?.const !== '0.3.0-alpha') fail('workflow_version const mismatch');
for (const key of ['workflow_version','research_plan','evidence_matrix','analysis_brief','diagnostics']) {
...
if (fixture.workflow_version !== '0.3.0-alpha') fail('fixture workflow version mismatch');
if (!plan || plan.plan_version !== '0.3.0-alpha') fail('fixture missing alpha research plan');
...
if (!fixture.analysis_brief || fixture.analysis_brief.brief_version !== '0.3.0-alpha') fail('fixture needs compiled analysis brief');
...
if (!fixture.diagnostics || fixture.diagnostics.diagnostics_version !== '0.3.0-alpha') fail('fixture needs diagnostics');
