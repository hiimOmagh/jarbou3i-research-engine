...
- Release-candidate readiness checks generate a concise lock report after hosted evidence review and archive validation.
...
The stable `v1.4.0-bio-alpha.1` pass promotes the locked stable-release baseline to the stable release line. No product behavior changes are included. Use `docs/stable-release-archive.md` for tag, archive, and generated-artifact rules.
...
## v1.4.0-bio prompt contract and sample gate
...
## v1.4.0-bio systems completeness diagnostics
...
## v1.4.0-bio smoke stability note
...
## v1.4.0-bio diagnostics export and replay fixture
...
Hosted evidence now produces a versioned archive named `hosted-demo-evidence-v1.4.0-bio.zip` after the screenshot and visible-text evidence directory passes review. The archive identity guard rejects stale or unversioned hosted evidence archives, validates `hosted-demo-metadata.json`, confirms every visible-text snapshot matches the runtime `meta[name="app-version"]`, and verifies the archive filename matches the current app version.
...
## v1.4.0-bio browser-core stability guard
...
## v1.4.0-bio evidence archive structure guard
...
## v1.4.0-bio stable-release readiness

The browser evidence chain now ends with `tests/stable-release-readiness-check.mjs`. After hosted evidence review and archive validation pass, the readiness gate writes:
...
These reports are generated artifacts and must be removed before the final workspace hygiene lock if they are produced locally outside the GitHub artifact upload flow.
## v1.4.0-bio stable tag preparation

The remote-validated RC baseline is promoted to the stable `v1.4.0-bio` line without product/runtime behavior changes. The browser evidence chain now produces `hosted-demo-evidence-v1.4.0-bio.zip` plus stable lock reports:
